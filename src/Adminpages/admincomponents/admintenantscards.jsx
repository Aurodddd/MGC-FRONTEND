...
export default function AdminTenantsCard() {
  ...
  const [tenants, setTenants] = useState([
    { unit: "101", name: "Lance Atendidas", phone: "09123456789", due: "September 15, 2025", totalBill: "₱10,000" },
    { unit: "102", name: "Matthew Karl Batista", phone: "09198765432", due: "September 1, 2025", totalBill: "₱12,500" },
    { unit: "103", name: "Aldjon Dela Cruz", phone: "09111222333", due: "September 20, 2025", totalBill: "₱9,800" },
  ]);
          ...
          <div className="hidden print-header-visible items-center justify-between border-b-2 border-black pb-4 mb-6 text-black">
            <div className="flex items-center gap-4">
              ...
              <div className="text-left">
                ...
                <p className="text-xs uppercase tracking-widest text-gray-600">Tenant Summary Report</p>
              </div>
            </div>
            ...
            <tbody>
              {filteredTenants.map((tenant, index) => (
                <tr key={index} className="bg-white text-black text-sm">
                  <td className="py-3 px-4 text-left border-b border-gray-100 font-bold">{tenant.unit}</td>
                  <td className="py-3 px-4 text-left font-semibold border-b border-gray-100">{tenant.name}</td>
                  <td className="py-3 px-4 text-left border-b border-gray-100">{tenant.phone}</td>
                  <td className="py-3 px-4 text-left border-b border-gray-100">{tenant.due}</td>
                  <td className="py-3 px-4 text-center border-b border-gray-100">{tenant.totalBill}</td>
                  <td className="py-3 px-4 text-center no-print border-b border-gray-100">
                    <div className="flex items-center justify-center gap-3">
                      <button onClick={() => navigate("/admintenantprof")} className="p-2 bg-[#d45f41] text-white rounded-lg active:scale-90"><FaEye size={16} /></button>
                      <button onClick={() => { setTenantToDelete(tenant); setShowDeleteModal(true); }} className="p-2 bg-[#4b150d] text-[#efd4c4] rounded-lg active:scale-90"><FaTrashAlt size={16} /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-center z-50 no-print">
          <div className="bg-white rounded-lg shadow-xl p-6 w-[90%] max-w-md text-center">
            <h2 className="text-xl font-semibold mb-4 text-[#4b150d]">Are you sure you want to delete this tenant?</h2>
            <p className="text-[#4b150d] mb-6">It will clear the tenant's details in their profile and remove them from their unit.</p>
            <div className="flex justify-end gap-4">
              <button onClick={() => setShowDeleteModal(false)} className="bg-gray-300 px-4 py-2 rounded-md cursor-pointer text-black">No</button>
              <button onClick={() => setShowDeleteModal(false)} className="bg-[#4b150d] text-[#efd4c4] px-4 py-2 rounded-md cursor-pointer">Yes</button>
            </div>
          </div>
        </div>
      )}
